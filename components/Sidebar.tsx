...
export default function Sidebar() {
  ...
  const { isAuthenticated, isSubscribed, user } = useSelector(
  ...
  const linkData = [
    ...
    { href: "/highlights", label: "Highlights", disabled: true },
    { href: "/search", label: "Search", disabled: true },
    { href: "/settings", label: "Settings" },
    { href: "/help", label: "Help & Support", disabled: true },
  ];

  const links: SidebarLink[] = linkData.map((link) => ({
    ...link,
    ...
    <>
      ...
      <div className="sidebar sidebar--closed">
        ...
        <div
          ...
          style={isPlayerPage ? { height: "calc(-140px + 100vh)" } : {}}
        >
            ...
            {links.slice(0, 4).map((link) => (
              <a
                ...
                className={`sidebar__link--wrapper ${
                  link.disabled ? "sidebar__link--not-allowed" : ""
                }`}
              >
                <div
                  className={`sidebar__link--line ${
                    isLinkActive(link.href) ? "active--tab" : ""
                  }`}
                ></div>
                <div className="sidebar__icon--wrapper">{link.icon}</div>
                <div className="sidebar__link--text">{link.label}</div>
              </a>
            ))}
            {/* Font size changer */}
            {pathname?.startsWith("/player/") && (
              <div className="sidebar__link--wrapper sidebar__font--size-wrapper">
                {([16, 18, 20, 22] as FontSize[]).map((size) => (
                  <div
                    key={size}
                  ...
                  >
                    <FontIcon
                      className={`sidebar__font--size-icon-${sizeLabels[size]}`}
                    />
                  </div>
                ))}
              </div>
